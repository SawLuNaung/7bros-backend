...
router.post("/create-driver", authenticateAdminToken, async (req, res) => {
    try {
        ...
        if (!driver_id || !name || !phone || !vehicle_number || !password) {
            return res.status(400).json({message: "Missing required fields (driver_id, name, phone, vehicle_number, password)"});
        }

        // 2. Validate password length (exactly 6 characters)
        ...
        if (existingDriverById) {
            return res.status(400).json({message: `Driver ID ${driver_id} is already in use`});
        }

        // 4. Check if driver with phone already exists
        ...
        // 6. Determine if driver should be verified
        ...
        // 7. Combine address fields for storage
        let address = null;
        ...
        // 8. Insert driver record (driver_id from user input)
        ...
        // 9. Return success response (HTTP 200 for Hasura actions)
        ...
        if (error.code === "23505") {
            return res.status(400).json({message: "Driver ID or phone number already exists"});
        }
